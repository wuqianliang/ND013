...
def draw_lines(img, lines, color=[255, 0, 0], thickness=2):
	...
	if right_y_set:
		...
		if SET_RFLAG >0:
			right_x_bottom = int(LAST_RIGHT[0] + (right_x_bottom - LAST_RIGHT[0]) * COR_DRIFT_WEIGHT)
			right_x_top = int(LAST_RIGHT[1] + (right_x_top - LAST_RIGHT[1]) * COR_DRIFT_WEIGHT)
			right_y_top = int(LAST_RIGHT[2] + (right_y_top - LAST_RIGHT[2]) * COR_DRIFT_WEIGHT)
		else:
			SET_RFLAG=1

		cv2.line(img, (right_x_top, right_y_top) , (right_x_bottom, max_y) , color, thickness)
		LAST_RSLOPE=rslope
		LAST_RIGHT=[right_x_bottom,right_x_top,right_y_top]
